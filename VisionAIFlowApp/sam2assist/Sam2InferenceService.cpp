#include "Sam2InferenceService.h"
...
bool Sam2InferenceService::initialize(const QString &modelDir)
{
    m_modelDir = modelDir;
    m_initialized = false;
    m_sam.reset();
    ...
    try
    {
        m_sam = std::make_shared<Sam2::SAM>(encoderEnginePath.toStdString(), decoderEnginePath.toStdString(), true, 0);
...
bool Sam2InferenceService::isInitialized() const
{
    return m_initialized;
}

QString Sam2InferenceService::modelDir() const
{
    return m_modelDir;
}

void Sam2InferenceService::setCurrentImage(const QImage &image)
{
...
Sam2InferResult Sam2InferenceService::inferByPoint(const QPointF &point) const
{
    return toInferByPrompt(&point, nullptr);
}

Sam2InferResult Sam2InferenceService::inferByRect(const QRectF &rect) const
{
    const QRectF normalizedRect = rect.normalized();
    return toInferByPrompt(nullptr, &normalizedRect);
}

bool Sam2InferenceService::isReady() const
{
    return m_initialized && (m_sam != nullptr);
}

QRectF Sam2InferenceService::toClampRect(const QRectF &rect) const
{
    if (m_currentImage.isNull())
    {
        return QRectF();
...
QString Sam2InferenceService::toResolveEncoderEnginePath(const QString &modelDir) const
{
    const QDir dir(modelDir);
    const QStringList candidates = {
        QStringLiteral("sam2_image_encode.fp16.trt"),
...
QString Sam2InferenceService::toResolveDecoderEnginePath(const QString &modelDir) const
{
    const QDir dir(modelDir);
    const QStringList candidates = {
        QStringLiteral("sam2_decode.fp16.trt"),
...
QImage Sam2InferenceService::toImageForInference(const QImage &image) const
{
    if (image.isNull())
    {
        return QImage();
...
cv::Mat Sam2InferenceService::toQImageAsBgrMat(const QImage &image) const
{
    if (image.isNull())
    {
        return cv::Mat();
...
Sam2InferResult Sam2InferenceService::toInferByPrompt(const QPointF *pointPrompt, const QRectF *rectPrompt) const
{
    Sam2InferResult result;
    ...
    try
    {
        ...
        result.success = true;
        result.roiData = toRectToRoiData(clampedBestRect);
        result.maskContour = toLargestContour(bestMask);
        result.minRectRoiData = toAxisAlignedRectRoiData(result.maskContour);
        if (result.minRectRoiData.size() < 8)
        {
            result.minRectRoiData = result.roiData;
        }
        return result;
    }
    catch (const std::exception &ex)
...
QVector<double> Sam2InferenceService::toRectToRoiData(const QRectF &rect)
{
    return {
        rect.left(), rect.top(),
        rect.right(), rect.top(),
...
cv::Mat Sam2InferenceService::toMaskBinaryU8(const cv::Mat &mask)
{
    if (mask.empty())
    {
        return cv::Mat();
...
QRectF Sam2InferenceService::toBoundingRectFromMask(const cv::Mat &mask)
{
...
QVector<QPointF> Sam2InferenceService::toLargestContour(const cv::Mat &mask)
{
...
QVector<double> Sam2InferenceService::toAxisAlignedRectRoiData(const QVector<QPointF> &contour)
{
