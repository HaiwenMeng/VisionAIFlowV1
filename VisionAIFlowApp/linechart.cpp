...
void LineChart::removeAll(int index)
{
    Q_ASSERT(index <= datas.size());
    if (datas.at(index).points.empty())
        return;
    datas[index].points.clear();
    startRangeAnimation();
}
...
void LineChart::paintEvent(QPaintEvent *event)
{
    ...
    /// 边界
    contentRect = QRect(paddings.left(),
                        paddings.top(),
    ...
    const int xMin = animatingXMin ? _animatedXMin : displayXMin, xMax = animatingXMax ? _animatedXMax : displayXMax;
    const int yMin = animatingYMin ? _animatedYMin : displayYMin, yMax = animatingYMax ? _animatedYMax : displayYMax;
    ...
    for (int i = 0; i < datas.size(); i++)
    {
        ...
        for (int j = 0; j < line.points.size(); j++)
        {
            ...
            const QPoint contentPt(contentRect.width() * (pt.x() - xMin) / (xMax - xMin),
                                   contentRect.height() * (pt.y() - yMin) / (yMax - yMin));
            const QPoint dispt(contentRect.left() + contentPt.x(), contentRect.bottom() - contentPt.y());
        ...
        if (pointDotType)
        {
            for (int i = 0; i < displayPoints.size(); i++)
            {
                ...
                QRect pointRect(pt.x() - pointDotRadius,
                                pt.y() - pointDotRadius,
                                pointDotRadius * 2,
                                pointDotRadius * 2);
    ...
    if (accessMinDis != 0x3f3f3f3f)
    {
        ...
        QRect pointRect(accessNearestPos.x() - pointDotRadius,
                        accessNearestPos.y() - pointDotRadius,
                        pointDotRadius * 2,
                        pointDotRadius * 2);
    ...
    else // 使用 xMin ~ xMax 的 int
    {
        ...
        int maxTextWidth =
            qMax(fm.horizontalAdvance(QString::number(xMin)), fm.horizontalAdvance(QString::number(xMax)));
        int displayCount =
            qMax((contentRect.width() + labelSpacing) / (maxTextWidth + labelSpacing), 1); // 最多显示多少个标签
        ...
        for (int i = xMin; i <= xMax; i += step)
        {
            ...
            if (hovering && contentRect.contains(accessNearestPos) &&
                (x + contentRect.left() >= accessNearestPos.x() - w &&
                 x + contentRect.left() <= accessNearestPos.x() + w))
            {
    ...
    for (int k = 0; k < datas.size() && k < 1; k++)
    {
        ...
        for (int i = yMin; i <= yMax; i += step)
        {
            ...
            if (hovering && contentRect.contains(accessNearestPos) &&
                (y >= accessNearestPos.y() - lineSpacing && y <= accessNearestPos.y() + lineSpacing))
            {
            ...
            if (k == 0) // 左边
            {
                painter.drawText(QPoint(contentRect.left() - labelSpacing - w - 5, y + lineSpacing / 2),
                                 QString::number(val) + "%");
            }
            else // 右边
            {
                painter.drawText(QPoint(contentRect.right() + labelSpacing, y + lineSpacing / 2),
                                 QString::number(val) + "%");
            }
            if (highlighted)
            {
...
void LineChart::enterEvent(QEnterEvent *event)
{
...
QPropertyAnimation *
LineChart::startAnimation(const QByteArray &property, int start, int end, bool *flag, int duration, QEasingCurve curve)
{
    *flag = true;
    ...
    connect(ani,
            ...
            this,
            [=](QPropertyAnimation::State state)
            {
                // 避免动画过程中的内存泄漏
