...
void translateResult(TrtSam3InferResult *result, double dx, double dy)
{
    if (result == nullptr)
    {
        return;
    }
    for (TrtSam3ObjectResult &object : result->objects)
    {
...
bool makeSmallTargetCrop(const QRectF &imageRect,
                         ...
                         QString *errorMessage)
{
    ...
    const int cropWidth =
        qMin(ceilToMultiple(prompt.width() * kSmallTargetCropFactor, kInferenceSizeMultiple), maxCropWidth);
    const int cropHeight =
        qMin(ceilToMultiple(prompt.height() * kSmallTargetCropFactor, kInferenceSizeMultiple), maxCropHeight);
    const QPointF center = prompt.center();
    const int left =
        qBound(0, static_cast<int>(std::round(center.x() - cropWidth * 0.5)), imageSize.width() - cropWidth);
    const int top =
        qBound(0, static_cast<int>(std::round(center.y() - cropHeight * 0.5)), imageSize.height() - cropHeight);
...
bool SamInferenceBridge::initialize(QString *errorMessage)
{
    ...
    if (!m_sam3)
    {
        m_sam3 = std::make_unique<TrtSam3>();
    }
...
void SamInferenceBridge::release()
{
    m_sam3.reset();
    m_initialized = false;
...
TrtSam3InferResult SamInferenceBridge::inferByPoint(const QPointF &imagePoint, const QString &labelName)
{
...
TrtSam3InferResult SamInferenceBridge::inferByRect(const QRectF &imageRect, const QString &labelName)
{
    TrtSam3InferResult result;
    if (!validateReady(&result))
    {
...
TrtSam3InferResult SamInferenceBridge::inferByRects(const QVector<QRectF> &imageRects, const QString &labelName)
{
    TrtSam3InferResult result;
    if (!validateReady(&result))
    {
...
TrtSam3InferResult SamInferenceBridge::inferSmallTargetByRect(const QRectF &imageRect, const QString &labelName)
{
    TrtSam3InferResult result;
    if (!validateReady(&result))
    {
...
bool SamInferenceBridge::validateReady(TrtSam3InferResult *result) const
{
    if (result == nullptr)
    {
        return false;
