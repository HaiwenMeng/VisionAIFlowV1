...
// ----------------------------------------------------------------------
YOLOv8Impl::YOLOv8Impl(const YoloV8NetworkConfig &config){

    size_t nc = config.inputChannels;
    size_t class_num = config.classCount;
    size_t reg_max = config.regMax;
    const std::string &model = config.variant;
    double depth, width;
    ...
    if (model == "yolov8n"){
        ...
        throw std::invalid_argument("YOLOv8 variant must be yolov8n, yolov8s, yolov8m, yolov8l or yolov8x");
    }
