...
foundation::Result<void> CreateUnsignedClassificationModelPackage(const QString &packageRoot, const QString &onnxPath, const ClassificationPackageMetadata &metadata)
{
    ...
    if (metadata.modelId.trimmed().isEmpty() || metadata.artifactContractVersion <= 0 || metadata.inputFeatures <= 0 || metadata.classCount < 2 || metadata.labels.size() != metadata.classCount) return foundation::Result<void>::Failure(foundation::Error::Create(foundation::ErrorCode::InvalidArgument, "Classification model package metadata is incomplete"));
    ...
    package.insert(QStringLiteral("modelId"), metadata.modelId);
    package.insert(QStringLiteral("artifactContractVersion"), metadata.artifactContractVersion);
...
foundation::Result<void> VerifyModelPackage(const QString &packageRoot, const bool requireSignature)
{
    ...
    if (packageObject.value(QStringLiteral("packageSchemaVersion")).toInt() != 1 || packageObject.value(QStringLiteral("modelId")).toString().isEmpty() || packageObject.value(QStringLiteral("artifactContractVersion")).toInt() <= 0 || packageObject.value(QStringLiteral("packageId")).toString().isEmpty() || packageObject.value(QStringLiteral("packageVersion")).toString().isEmpty() || packageObject.value(QStringLiteral("modelFamily")).toString().isEmpty() || packageObject.value(QStringLiteral("adapterId")).toString().isEmpty() || packageObject.value(QStringLiteral("adapterVersion")).toString().isEmpty() || (projectType != QStringLiteral("classification") && projectType != QStringLiteral("detection")) || !packageObject.value(QStringLiteral("artifacts")).isArray() || !packageObject.value(QStringLiteral("inputContract")).isObject() || !packageObject.value(QStringLiteral("outputContract")).isObject()) return foundation::Result<void>::Failure(foundation::Error::Create(foundation::ErrorCode::ProtocolViolation, "Model package manifest does not satisfy a supported package schema"));
...
foundation::Result<ModelPackageRuntimeContract> LoadVerifiedModelPackageRuntimeContract(const QString &packageRoot, const QString &runtimeId, const bool requireSignature)
{
    if (runtimeId != QStringLiteral("openvino_cpu") && runtimeId != QStringLiteral("trt1001_opset12")) return foundation::Result<ModelPackageRuntimeContract>::Failure(foundation::Error::Create(foundation::ErrorCode::UnsupportedOperation, "Model package runtime id is unsupported"));
    const auto verified = VerifyModelPackage(packageRoot, requireSignature);
    if (!verified.IsSuccess()) return foundation::Result<ModelPackageRuntimeContract>::Failure(verified.Failure());
    const auto package = ReadObject(QDir(packageRoot).filePath(QStringLiteral("package.json")));
    if (!package.IsSuccess()) return foundation::Result<ModelPackageRuntimeContract>::Failure(package.Failure());
    const QJsonObject object = package.Value();
    const QString artifactRelative = QStringLiteral("artifacts/") + runtimeId + QStringLiteral("/model.onnx");
    if (!object.value(QStringLiteral("artifacts")).toArray().contains(artifactRelative) || !QFileInfo(QDir(packageRoot).filePath(artifactRelative)).isFile()) return foundation::Result<ModelPackageRuntimeContract>::Failure(foundation::Error::Create(foundation::ErrorCode::ProtocolViolation, "Model package runtime artifact is missing or not declared"));
    ModelPackageRuntimeContract contract;
    contract.packageRoot = packageRoot;
    contract.modelId = object.value(QStringLiteral("modelId")).toString();
    contract.adapterVersion = object.value(QStringLiteral("adapterVersion")).toString();
    contract.projectType = object.value(QStringLiteral("projectType")).toString();
    contract.decoderId = object.value(QStringLiteral("decoderId")).toString();
    contract.artifactContractVersion = object.value(QStringLiteral("artifactContractVersion")).toInt();
    contract.artifactPath = QDir(packageRoot).filePath(artifactRelative);
    if (contract.modelId.isEmpty() || contract.adapterVersion.isEmpty() || contract.projectType.isEmpty() || contract.decoderId.isEmpty() || contract.artifactContractVersion <= 0) return foundation::Result<ModelPackageRuntimeContract>::Failure(foundation::Error::Create(foundation::ErrorCode::ProtocolViolation, "Model package runtime contract is incomplete"));
    return foundation::Result<ModelPackageRuntimeContract>::Success(std::move(contract));
}
