...
foundation::Result<void> AsyncClassificationJob::Start(const AsyncClassificationJobConfig &config, const torch::Tensor &features, const torch::Tensor &targets)
{
    ...
    try
    {
        if (!config.modelFactory) return foundation::Result<void>::Failure(foundation::Error::Create(foundation::ErrorCode::InvalidArgument, "Async classification job requires an adapter-provided model factory"));
        const auto model = config.modelFactory(config.inputFeatures, config.classCount);
