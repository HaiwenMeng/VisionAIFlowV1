...
{
    const bool allowed = (from == JobState::Created && to == JobState::Validating) ||
                         ...
                         (from == JobState::Starting &&
                          (to == JobState::Running || to == JobState::Failed || to == JobState::Cancelling)) ||
                         (from == JobState::Running && (to == JobState::Pausing || to == JobState::Cancelling ||
                                                        to == JobState::Completing || to == JobState::Failed)) ||
                         (from == JobState::Pausing && (to == JobState::Paused || to == JobState::Failed)) ||
                         (from == JobState::Paused &&
                          (to == JobState::Running || to == JobState::Cancelling || to == JobState::Failed)) ||
