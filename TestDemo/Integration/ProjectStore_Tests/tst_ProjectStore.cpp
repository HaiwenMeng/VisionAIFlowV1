...
private slots:
    ...
    void MigratesSchemaOneProjectWithoutModelBinding();
...
void ProjectStoreTest::CreatesAndReopensEveryProjectType()
{
    ...
    for (const auto &[type, mode] : types)
    {
        ...
        QCOMPARE(reopened.Value().modelId, definition.modelId);
    }
}

void ProjectStoreTest::MigratesSchemaOneProjectWithoutModelBinding()
{
    QJsonObject legacy{{QStringLiteral("schemaVersion"), 1}, {QStringLiteral("projectId"), QUuid::createUuid().toString(QUuid::WithoutBraces)}, {QStringLiteral("name"), QStringLiteral("legacy")}, {QStringLiteral("projectType"), QStringLiteral("detection")}, {QStringLiteral("classificationMode"), QStringLiteral("not_applicable")}, {QStringLiteral("createdUtc"), QDateTime::currentDateTimeUtc().toString(Qt::ISODateWithMs)}};
    const auto migrated = visionaiflow::project_store::MigrateProjectJson(legacy);
    QVERIFY2(migrated.IsSuccess(), migrated.IsSuccess() ? "" : migrated.Failure().message.c_str());
    QCOMPARE(migrated.Value().project.value(QStringLiteral("schemaVersion")).toInt(), visionaiflow::project_store::CurrentProjectSchemaVersion);
    QCOMPARE(migrated.Value().project.value(QStringLiteral("modelId")).toString(), QString());
    QCOMPARE(migrated.Value().appliedSourceVersions, QVector<int>{1});
}
...
void ProjectStoreTest::RejectsUnsupportedProjectSchemaVersions()
{
    ...
    const QString futureRoot = createAndRewriteProject(QStringLiteral("future"), 3);
