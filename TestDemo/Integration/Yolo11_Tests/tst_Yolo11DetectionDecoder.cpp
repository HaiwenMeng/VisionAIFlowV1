...
private slots:
    ...
    void RegistersAllBuiltInAdapters();
...
void Yolo11DetectionDecoderTest::RegistersAllBuiltInAdapters()
{
    visionaiflow::models::api::ModelRegistry registry;
    const auto linearRegistered = visionaiflow::models::classification::linear::RegisterLinearClassificationAdapter(registry);
    QVERIFY2(linearRegistered.IsSuccess(), linearRegistered.IsSuccess() ? "" : linearRegistered.Failure().message.c_str());
    const auto yoloRegistered = visionaiflow::models::yolo11::RegisterYolo11DetectionAdapters(registry);
    QVERIFY2(yoloRegistered.IsSuccess(), yoloRegistered.IsSuccess() ? "" : yoloRegistered.Failure().message.c_str());
    QVERIFY(registry.Create(QStringLiteral("classification.linear.v1"), visionaiflow::domain::ProjectType::Classification).IsSuccess());
    QVERIFY(registry.Create(QStringLiteral("detection.yolo11.tiny.v1"), visionaiflow::domain::ProjectType::Detection).IsSuccess());
    QVERIFY(registry.Create(QStringLiteral("detection.yolo11.grid.v1"), visionaiflow::domain::ProjectType::Detection).IsSuccess());
    QVERIFY(!registry.Create(QStringLiteral("model.unknown.v1"), visionaiflow::domain::ProjectType::Detection).IsSuccess());
}
...
void Yolo11DetectionDecoderTest::CreatesYolo11DetectionModelPackage()
{
    ...
    metadata.modelId = QStringLiteral("detection.yolo11.grid.v1");
