...
class Q_DECL_EXPORT YtQString : public YtVarBassX
{
public:
    //有link表示符号的info就会起到索引作用，方便传递数值用的很重要
    YtQString(QString bValue = QString(), QString strVarInfo = "",int DataType=YtVal);
    ~YtQString();
public:
    ...
    QString toQString();
    void toSetQString(QString bValue);
