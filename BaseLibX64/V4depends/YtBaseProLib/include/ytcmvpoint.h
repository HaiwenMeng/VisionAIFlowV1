...
class Q_DECL_EXPORT YtCMvPoint : public YtVarBassX
{
public:
    //有link表示符号的info就会起到索引作用，方便传递数值用的很重要
    YtCMvPoint(CMvPoint bValue = CMvPoint(), QString strVarInfo = "",int DataType=YtVal);
    ~YtCMvPoint();
public:
    ...
    CMvPoint toCMvPoint();
    void toSetCMvPoint(CMvPoint bValue);
...
private:
    CMvPoint m_Value;//当前值
    CMvPoint m_ValueShoot;//快照数值
};
#endif // YtCMvPoint_H
