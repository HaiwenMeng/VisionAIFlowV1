...
class Q_DECL_EXPORT YtDouble : public YtVarBassX
{
public:
    //有link表示符号的info就会起到索引作用，方便传递数值用的很重要
    YtDouble(double bValue = 0.0, QString strVarInfo = "",int DataType=YtVal);
    ~YtDouble();
public:
    ...
    double toDouble();
    void toSetDouble(double bValue);
