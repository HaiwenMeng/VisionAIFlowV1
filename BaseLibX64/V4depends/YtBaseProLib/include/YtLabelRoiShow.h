#ifndef YTLABELROISHOW_H
#define YTLABELROISHOW_H
...
namespace Ui {
class YtLabelRoiShow;
}

class QDESIGNER_WIDGET_EXPORT YtLabelRoiShow : public QWidget
{
    Q_OBJECT

public:
    explicit YtLabelRoiShow(QWidget *parent = nullptr);
    ~YtLabelRoiShow();

private:
    Ui::YtLabelRoiShow *ui;
public:
     //固定绘制 体系
...
signals:
    ...
    void SigGetDataUpdate(QString SetKey,QString LableName,int type);//只要返回Id和类型以及标签名称
public:
    //ROI 体系
...
public:
     QList<QString> toGetKeyRoi();
     bool toGetKeyData(QString key,int &ShapeType,QVector<double> &OutData);
public:
     void toSetLabelNames(QStringList Names,QVector<QColor> SetColor);
     void toSetRoiDefaltType(int AddType);//设置默认ROI模式
     void toSetRoiDefaltName(QString SetName);//设置默认ROI模式
     void toSetItemKey(QString Key, QString Name);
     void toAppItemLabel(QString Name,int Shape,QVector<double> SetData);//增加数据
     void toClearMbutton();

};

#endif // YTLABELROISHOW_H
